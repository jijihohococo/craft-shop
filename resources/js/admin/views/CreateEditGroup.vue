		...
		<div class="container-fluid">
			<div class="card card-default">
				<div class="card-header">
					<h3 class="card-title">{{ isNaN(this.$route.params.id) ? "Create Group" : "Update Group" }}</h3>
				</div>
				...
				<form v-else-if="actions[current]" @submit.prevent=" !isNaN(this.$route.params.id) ? updateGroup() : createGroup()">
					<div class="card-body">
						<div class="row">
							<div class="col-md-12">
...
<script >
	...
	export default {
		...
		methods : {
			createGroup(){
				window.axios.post("groups",this.fields).then( (response) => {
			...
			updateGroup(){
				window.axios.put(`groups/${this.$route.params.id}`,this.fields).then( (response) => {
					if(response.data.message=='Loading'){
			...
			async getGroupData( groupId ){
				window.axios.get('groups/'+groupId + '/edit' ).then((response) => {
					if(response.data.message=='Loading'){

						showSwalLoading(this);
					}else{
						this.fields=response.data.group;
					}
				} ).catch( (error) => {
					errorResponse(error,this,'update')
				} )
