                ...
                <div class="card">
                  <div class="card-header row">
                    <div v-if="actions.read" class="card-tools col-8 mt-1">
                        <form v-on:submit.prevent="searchPermissions(1)">
...
<script >
    ...
    methods :{
        ...
        getPermissions(page){
            ...
            window.axios.get("permissions?page=" + page ).then(( response ) =>  {
     ...
     searchPermissions(page){
        this.currentPage=page;
        window.axios.get('permission_search?search=' + this.search + '&page=' + page ).then( (response) => {
            if(response.data.message=='Loading'){
